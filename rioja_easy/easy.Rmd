...
source("~/cHAC/implemented_method.R")
...
We do not calculate Pseudo-Euclidean (PE) embedding here, instead we calculate the distacne metric by ourself and fit it in rioja inbuilt functions

For ward's linkage: the data can be misleading as the varience of the clusters are calculated by euclidean method

For SLINK method: The data have low varience hence cant seperate into clusters
